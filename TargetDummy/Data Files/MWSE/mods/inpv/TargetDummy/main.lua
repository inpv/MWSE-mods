...
local function getHitSuccess(skill)
    ...
    --mwse.log("fatigueTerm %s", fatigueTerm)
    --attack term
    ...
    --mwse.log("attackTerm %s", attackTerm)
    --roll for hit
    local rand = math.random(100)
    --mwse.log("rand: %s", rand)
    local didHit = ( attackTerm > 0 and rand < attackTerm )
    --mwse.log("didHit: %s", didHit)
    return didHit
end
            ...
            if meleeSkill then
                if getHitSuccess(meleeSkill) then
                    ...
                    if tes3.mobilePlayer.readiedWeapon then
                        ...
                        tes3.playSound{ sound = soundIds.h2h}
                    end
                else
